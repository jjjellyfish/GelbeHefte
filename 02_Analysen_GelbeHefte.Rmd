---
title: "Ergebnisse aus den Gelben Heften"
output: 
  pdf_document:
...
```{r data}
load("O:/U4951/Consulimus_Abschlussdatenlieferung/01_GelbeHefte/gh_clean.rda")
```
...
kbl2 = function(...) {
  kableExtra::kbl(..., booktabs = T, digits=1, 
                  format.args = list(decimal.mark = ',', big.mark = "."), linesep = "") %>% 
...
rel_freq <- function(df) {
  ...
  df <- df %>% 
    ...
    mutate_at(2:length(.), ~ sprintf("%.1f", round(., 1))) %>% 
    mutate_at(vars(2:length(.)), ~gsub( '\\.', ',', ., perl=TRUE )) 
}
...
openxlsx::write.xlsx(tmp, file='03_Ergebnisse/Ruecklauf.xlsx')
```
...
##### Ausgewählte Auffälligkeiten der Augen in der U2–U3
 \newline
...
##### Ausgewählte Auffälligkeiten der Augen in der U4–U7
 \newline
...
##### Ausgewählte Auffälligkeiten der Augen in der U7a–U9
 \newline
...
tmp$Ges.[3] <- tmp$Ges.[1] +tmp$Ges.[2]
tmp[tmp$`Kreuz gesetzt`=='valides n' ,2:ncol(tmp)] <- 
  tmp[tmp$`Kreuz gesetzt`=='Ja' ,2:ncol(tmp)] + tmp[tmp$`Kreuz gesetzt`=='Nein' ,2:ncol(tmp)]
...
##### Ausgewählte Auffälligkeiten der Zähne und der Schleimhaut in der U6
 \newline
...
  \linebreak
Häufigkeit von erweitertem Beratungsbedarf mit Verweis in zum Zahnarzt zur zahnärztlichen Früherkennungsuntersuchung 
```{r Beratung_Zahnarzt}
tmp <- abs_freq(df=dat, x='Beratung_Zahnarzt') 
...
```{r Auffälligkeiten}
# Variable umbenennen
names(dat) <- str_replace(names(dat), "Auffaelligkeiten_Freitext", "Auff_Freitext")
names(dat) <- str_replace(names(dat), "morphologische_Auffaelligkeiten", "morphologische_Auff")
names(dat) <- str_replace(names(dat), "keineAuffaelligkeiten", "keineAuff")


tmp <- abs_freq(df=dat, x='Auffaelligkeiten') 
...
Anzahl vollständig unlesbarer Freitexte zu relevanten anamnestischen Ergebnissen
```{r Anamnese_Freitext_n_unlesbar}
tmp <- tmp %>% na.omit() %>% arrange(u, var)

### Vorhandensein ausgeben
unlesbar <- c('^\\[unlesbar\\]$', '^\\[Unlesbar\\]$','^\\[unlesbar\\]\\.$', '^\\[nicht lesbar \\(gleich wie bei U2,U3\\)\\]$', '^\\[unkenntlich\\]$', '^\\(nicht lesen\\)$', '^\\(nicht lesbr\\)$')

tmp2 <- tmp %>%
  mutate(nicht_lesbar=case_when(
    grepl(paste(unlesbar, collapse='|'), var) ~ 'Ja', 
    TRUE ~ 'Nein')) %>% 
  group_by(u, nicht_lesbar) %>%
  tally() %>%
  pivot_wider(., names_from = u, values_from = n) %>%
  janitor::adorn_totals(c('row', 'col'), name = c('Ges.','Ges.'), na.rm = TRUE) %>%
  untabyl()
...
## absolute Tabelle
rowline_at <- nrow(tmp2)-1
tmp2 %>% kbl2(col.names= c('Eintrag vollständig unlesbar', paste(names(.)[2:length(.)], '[n]', sep=' '))) %>% 
  row_spec(rowline_at, extra_css = "border-bottom: 1px solid", hline_after=TRUE)
...
tmp2 %>% kbl2(col.names= c('Eintrag vollständig unlesbar', paste(names(.)[2:length(.)], '[%]', sep=' ')), 
              align = align_string(tmp2)) %>% 
  row_spec(rowline_at, extra_css = "border-bottom: 1px solid", hline_after=TRUE)
```
...
Freitexte zu relevanten anamnestischen Ergebnissen (nur aufgeführt, wenn zumindest teilweise lesbar)
```{r Anamnese_Freitext_text}
tmp2 <- tmp %>% filter(!grepl(paste(unlesbar, collapse='|'), var))
...
#   #kableExtra::column_spec(2, width = "15cm") # funktioniert im Chunk, wird aber nicht geknittet

tbl <- tmp2 %>% select(u, var) 
colnames(tbl) <- c('U-Untersuchung', 'Relevante anamnestische Ergebnisse')
tbl %>% pander::pander(justify = 'left', split.cells = c(1, 59))
```
...
```{r Beratung}
names(dat) <- str_replace(names(dat), "Beratung$", "Beratung_Abschnitt")
tmp <- abs_freq(df=dat, x='Beratung_Abschnitt', cat_nr=5) 
...
vars <- c('KGewicht', 'KLaenge', 'Kopfumfang', 'keineAuff', 
          'Auffaelligkeiten', 'VitaminK_ja', 
          'VitaminK_Dosis_2mg', 'VitaminK_abwDosis', 'VitaminK_nein')

# Datensatz zurechtschneiden
tmp <- dat %>% 
  select(GH_ID, contains('ausgefüllt'), matches(vars), # Kopien müssen ausgefüllt sein
...
del <- tmp %>% filter_at(vars(2:ncol(.)), any_vars(grepl("99", .))) # Ergebnisseite muss vorliegen
...
tmp <- tmp %>% 
  mutate(
    ...
    auff = case_when(
      keineAuff == 1 | Auffaelligkeiten == 1 ~ 1,
      TRUE ~ 0
    ),
    vitk_easy = case_when(
...
tmp2$variables <- c('Körpergewicht dokumentiert', 
                    ...
                    'Vitamin K dokumentiert (streng)', 
                    'U-Untersuchung lückenlos (vereinfacht)', 
                    'U-Untersuchung lückenlos (streng)')
...
## relevante Variablen filtern & Datensatz vorbereiten
vars <- c('OBE_altersgem','KGewicht', 'KLaenge', 'Kopfumfang', 'keineAuff', 
...
tmp2$variables <- c('Ergebnis OBE dokumentiert',
                    ...
                    'Impfstatus dokumentiert (streng)',
                    'U-Untersuchung lückenlos (vereinfacht)', 
                    'U-Untersuchung lückenlos (streng)')
...
## relevante Variablen filtern & Datensatz vorbereiten
vars <- c('OBE_altersgem','KGewicht', 'KLaenge', 'Kopfumfang', 'BMI','keineAuff', 
          'Auffaelligkeiten', 'Impfstatus_ja','Impfstatus_nein', 'Impfstatus_fehlend')
...
## relevante Variablen filtern & Datensatz vorbereiten
vars <- c('OBE_altersgem','KGewicht', 'KLaenge', 'BMI','keineAuff', 
          'Auffaelligkeiten', 'Impfstatus_ja','Impfstatus_nein', 'Impfstatus_fehlend')
...
Untersuchung lückenlos (strenge Definition)
```{r streng, message=FALSE, warning=FALSE}
#abs
lueckl %>% 
  slice_tail(n=2) %>% 
  select(- Feld, -contains('%')) %>% 
...
```{r Lueckenlosigkeit_plot, message=FALSE, warning=FALSE, echo=FALSE}
# tmp <- lueckl %>% 
#   mutate(def = c(rep('vereinfachte Definition',2), rep('strenge Definition',2))) %>% 
#   select(ausgefüllt, def, contains('%')) %>% 
#   filter(ausgefüllt == 'Ja') %>% 
#   pivot_longer(cols=contains('%'), names_to = 'U', values_to = 'freq') %>% 
#   mutate(
#     #U = stringi::stri_sub(U,1,regexpr(" ",U)[[1]]-1),
#     U = str_extract(U, "[^\\ ]+"), 
#     freq=freq/100)
# 
# #manuell beschriften
# tmp$U_n <- rep(c('U2\n(n=1.708)', 'U3\n(n=1.700)', 'U4\n(n=1.496)', 'U5\n(n=1.182)', 'U6\n(n=896)', 'U7\n(n=711)', 'U7a\n(n=507)', 'U8\n(n=306)', 'U9\n(n=107)'),2)
# 
# p <- ggplot(tmp, aes(x = U_n, 
#                 y = freq, 
#                 fill = factor(def, levels=c('vereinfachte Definition', 'strenge Definition')), 
#                 #color= factor(def, levels = c('vereinfacht', 'streng')),
#                 label = scales::percent(freq, accuracy=.1, decimal.mark = ","))) + 
#   geom_bar(stat = "identity", position='dodge') +
#   scale_y_continuous(limits = c(0.0, 1.02), labels = scales::percent_format(), breaks=c(0,.2,.4,.6,.8,1)) +
#   geom_text(position = position_dodge(width = .9),vjust = -0.5, size = 4, color="#00386a") + 
#   scale_fill_manual(values=c('#00386a', '#BED600')) +
#   xlab("Untersuchung") +
#   ylab("Anteil lückenloser Untersuchungen")+
#   theme_bw() +
#   theme(axis.title.x = element_text(color="#00386a", family = "Verdana", size = 14),
#         axis.text.x= element_text(color="#00386a", family = "Verdana", size = 12),
#         #axis.ticks.x = element_text(color="#00386a", family = "Verdana", size = 14),
...
# 
# 
# bmp(file="03_Ergebnisse/Lueckenlosigkeit.bmp", width = 750, height = 500, type="cairo")
# print(p)
# dev.off()
```
...
```{r Ausfuellstand_Flowchart, warning=FALSE, message=FALSE}
...
tmp <- as.data.frame(t(tmp)) 
 rownames(tmp) <- c('', 'Kopien liegen vor', 'irgendwelche Eintragungen liegen vor', 'Ergebnisseite liegt vor', 
                   'U lückenlos ausgefüllt (vereinfacht)', 'U lückenlos ausgefüllt (streng)')
...
# fehlende lückenlos ausgefüllte Us identifizieren (streng)
mis_hard <- tmp %>% 
  group_by(GH_ID) %>%
  filter(!any(results == 0)) %>%
...
# Tabellenoutput putzen 
tmp2$Text <- c('Übermittelte Gelbe Hefte',
               'Kopien liegen für alle Us vor', 
               'irgendwelche Eintragungen liegen für alle Us vor', 
               'Ergebnisseite liegt für alle Us vor', 
               'alle Us lückenlos ausgefüllt (vereinfacht)', 
               'alle Us lückenlos ausgefüllt (streng)')
...
#openxlsx::write.xlsx(tmp2, file='03_Ergebnisse/Lueckl_Heft.xlsx')
```


\pagebreak
In wie vielen Heften fehlen wie viele U-Untersuchungen? (Unterteilt danach, bei welcher Stufe das Fehlen auftritt. Bei jeder folgenden Stufe werden nur Gelbe Hefte berücksichtigt, die bis dahin lückenlos waren. n = Anzahl Gelbe Hefte)
...
tmp <- lueckl_heft_u2u7a %>%
  ...
  #select(-contains('lueckl'), -contains('u_')) %>% 
  distinct(GH_ID, .keep_all = TRUE) %>% 
  ungroup()


tbl1 <- tabyl(tmp$mis_copy[tmp$mis_copy > 0]) %>% rename(anzahl = 1) %>% add_row(anzahl=7, n=0, percent=0)
tbl2 <- tabyl(tmp$mis_data2[tmp$mis_data2 > 0 & tmp$lueckl_copy==1]) %>% rename(anzahl = 1)
tbl3 <- tabyl(tmp$mis_results2[tmp$mis_results2 > 0 & tmp$lueckl_data==1], show_na = FALSE) %>% rename(anzahl = 1)
tbl4 <- tabyl(tmp$mis_easy2[tmp$mis_easy2 > 0 & tmp$lueckl_res==1], show_na = FALSE) %>% rename(anzahl = 1)
tbl5 <- tabyl(tmp$mis_hard2[tmp$mis_hard2 > 0 & tmp$lueckl_res==1],show_na = FALSE) %>% rename(anzahl = 1)

# Gesamttabelle
tbl <- list(tbl1, tbl2, tbl3, tbl4, tbl5) %>% 
  purrr::reduce(left_join, by = "anzahl") %>% 
...
Welche spezifischen U-Untersuchungen fehlen wie häufig?  (Unterteilt danach, bei welcher Stufe das Fehlen auftritt. Bei jeder folgenden Stufe werden nur Gelbe Hefte berücksichtigt, die bis dahin lückenlos waren. n = Anzahl U-Untersuchungen; die Prozent-Angaben beziehen sich auf die in der Vorstufe vorliegende Anzahl von Gelben Heften: n=231, n=203, n=199, n=180, n=180)
...
tbl2 <- tabyl(tmp$u[tmp$data==0 & tmp$lueckl_copy==1], show_missing_levels = TRUE)
tbl3 <- tabyl(tmp$u[tmp$results==0 & tmp$lueckl_data==1], show_missing_levels = TRUE, show_na = FALSE)
tbl4 <- tabyl(tmp$u[tmp$ges_easy==0 & tmp$lueckl_res==1], show_na = FALSE)
tbl5 <- tabyl(tmp$u[tmp$ges_hard==0 & tmp$lueckl_res==1], show_na = FALSE)

tbl <- bind_cols(tbl1, tbl2[2:length(tbl2)], tbl3[2:length(tbl3)], tbl4[2:length(tbl4)], tbl5[2:length(tbl5)])

# Nenner austauschen (Rueckmeldung haa: Bezug auf valide Kopien)
tbl <- tbl %>% 
  mutate(percent...3 = n...2 / 231, 
         percent...5 = n...4 / 203,
         percent...7 = n...6 / 199, 
         percent...9 = n...8 / 180, 
         percent...11 = n...10 / 180)
...
openxlsx::write.xlsx(tbl, file='03_Ergebnisse/Lueckl_Heft_welcheUs.xlsx')
```


```{r Lueckenlosigkeit_Heft_verloreneHefte, warning=FALSE, message=FALSE}
# Wie häufig fehlen bestimmte U-Untersuchungen in Folge
tmp <- lueckl_heft_u2u7a %>% select(GH_ID:data) %>% 
  pivot_wider(names_from=u, values_from=c(kopien, data))

tmp1 <- tmp %>% group_by(GH_ID) %>% 
  mutate(missc_u2 = kopien_U2, 
         missc_u2u3 = ifelse(kopien_U2==0 & kopien_U3==0, 0, 1),
         missc_u2u3u4 = ifelse(kopien_U2==0 & kopien_U3==0 & kopien_U4==0, 0, 1),
         missc_u2u3u4u5 = ifelse(kopien_U2==0 & kopien_U3==0 & kopien_U4==0 & 
                                   kopien_U5==0, 0, 1),
         missc_u2u3u4u5u6 = ifelse(kopien_U2==0 & kopien_U3==0 & kopien_U4==0 & 
                                     kopien_U5==0 & kopien_U6==0, 0, 1), 
         missc_u2u3u4u5u6u7 = ifelse(kopien_U2==0 & kopien_U3==0 & kopien_U4==0 & 
                                     kopien_U5==0 & kopien_U6==0 & kopien_U7==0, 0, 1)) %>% 
  select(GH_ID, starts_with('missc')) %>% 
  mutate_at(vars(missc_u2:missc_u2u3u4u5u6u7), as.factor) %>% 
  ungroup() 

tbl1 <- tmp1 %>% count(missc_u2)
tbl2 <- tmp1 %>% count(missc_u2u3)
tbl3 <- tmp1 %>% count(missc_u2u3u4)
tbl4 <- tmp1 %>% count(missc_u2u3u4u5)
tbl5 <- tmp1 %>% count(missc_u2u3u4u5u6)
tbl6 <- tmp1 %>% count(missc_u2u3u4u5u6u7)

tblc <- bind_cols(tbl1, tbl2[2], tbl3[2], tbl4[2], tbl5[2], tbl6[2])
tblc <- as.data.frame(t(tblc))
tblc <- tblc %>% filter(V1 != 1) %>% select(V2, V1)

tmp2 <- tmp %>% group_by(GH_ID) %>% 
  mutate(missd_u2 = data_U2, 
         missd_u2u3 = ifelse(data_U2==0 & data_U3==0, 0, 1),
         missd_u2u3u4 = ifelse(data_U2==0 & data_U3==0 & data_U4==0, 0, 1),
         missd_u2u3u4u5 = ifelse(data_U2==0 & data_U3==0 & data_U4==0 & 
                                   data_U5==0, 0, 1),
         missd_u2u3u4u5u6 = ifelse(data_U2==0 & data_U3==0 & data_U4==0 & 
                                     data_U5==0 & data_U6==0, 0, 1), 
         missd_u2u3u4u5u6u7 = ifelse(data_U2==0 & data_U3==0 & data_U4==0 & 
                                     data_U5==0 & data_U6==0 & data_U7==0, 0, 1)) %>% 
  select(GH_ID, starts_with('missd')) %>% 
  mutate_at(vars(missd_u2:missd_u2u3u4u5u6u7), as.factor) %>% 
  ungroup() 

tbl1 <- tmp2 %>% count(missd_u2)
tbl2 <- tmp2 %>% count(missd_u2u3)
tbl3 <- tmp2 %>% count(missd_u2u3u4)
tbl4 <- tmp2 %>% count(missd_u2u3u4u5)
tbl5 <- tmp2 %>% count(missd_u2u3u4u5u6)
tbl6 <- tmp2 %>% count(missd_u2u3u4u5u6u7)

tbld <- bind_cols(tbl1, tbl2[2], tbl3[2], tbl4[2], tbl5[2], tbl6[2])
tbld <- as.data.frame(t(tbld))
tbld <- tbld %>% filter(V1 != 1) %>% select(V2, V1)

tbl <-  bind_cols(tblc, tbld)

tbl <- tbl %>% 
  mutate_at(vars(V2...1:V1...4), as.numeric) %>%
  mutate(V1...2 = (V2...1 / 231)*100,
         V1...4 = (V2...3 / 231)*100)

rownames(tbl) <- c('U2 fehlt', 'U2 bis U3 fehlen', 'U2 bis U4 fehlen', 
                   'U2 bis U5 fehlen', 'U2 bis U6 fehlen', 'U2 bis U7 fehlen')

tbl %>% kbl2(col.names= c(rep(c('[n]', '[%]'),2))) %>% 
  add_header_above(c("", "Kopien" = 2, "Eintragungen" = 2))

openxlsx::write.xlsx(tbl, file='03_Ergebnisse/Lueckl_Heft_verloreneHefte.xlsx')
```



## 4.7	Wahrnehmung des Gelben Hefts durch Eltern

### 4.7.4	Notizen der Eltern 

Häufigkeit von Einträgen der Eltern in den Notizen im Abschnitt *Elterninformation* (Die hohe Anzahl fehlender Werte kommt dadurch zustande, dass die entsprechende Seite häufig nicht übermittelt wurde.)
```{r Elterninfo}
# Variable umbenennen
...
Analyse der Freitexteinträge der Eltern in den Notizen im Abschnitt *Elterninformation*
```{r Elterninfo_Freitext}
tmp <- openxlsx::read.xlsx("O:/U4951/Consulimus_Abschlussdatenlieferung/01_GelbeHefte/02_freitext_data/Freitexte_NotizenEltern_dkt.xlsx",sheet = 'rdata')

tmp <- tmp %>% mutate(freq_F=freq_F*100,
               freq_N=freq_N*100) 

tmp$freq_F[10] <- NA

tmp%>% 
  kbl2(col.names= c('Antwort', 'n', '% U-Untersuchungen', '% Nennungen')) %>% 
  row_spec(c(3, 8), extra_css = "border-bottom: 1px solid", hline_after=TRUE)
```
