...
tmp2 <- tmp %>% 
  summarize(lueckl_copy=sum(lueckl_copy, na.rm=TRUE), 
...
tmp2 <- tmp2 %>% 
  ...
  mutate(freq_ges = V1 / nrow(dat)*100,
         freq_copy = V1 / V1[1]*100,
         freq_data = V1 / V1[2]*100,
         freq_res = V1 / V1[3]*100) %>%
  mutate_at(vars(freq_ges:freq_res), ~ replace(., .>100, NA))


# Tabellenoutput putzen 
tmp2$Text <- c('Kopien liegen lückenlos vor', 'irgendwelche Eintragungen liegen lückenlos vor', 
