...
## LESEZEICHEN
Fallzahlen nach einzelnen U-Untersuchungen ausgeben

```{r einzelne_us}
tmp <- dat %>% group_by()

# df <- df %>% select(GH_ID, contains('ausgefüllt'), contains({x})) %>% 
#     pivot_longer(contains({x}), names_to='u', values_to = "var") 
```

- weitere Gruppierungen? 
z.B. Anzah Hefte, die mit bestimmter U anfangen
...
tmp %>% 
  select(u, var) %>% 
  kbl2(col.names= c('U-Untersuchung', 'Relevante anamnestische Ergebnisse'))
...
Für Kinder, die zum Zeitpunkt der Datenerhebung die U-Untersuchungen U8 und U9 durchlaufen haben (= die letzte dokumentierte U-Untersuchung ist eine U8 bzw. U9) kann nicht sichergestellt werden, dass sie bei der Geburt ein gemäß der 2016 in Kraft getretenen Neufassung der Kinder-Richtlinie überarbeitetes Gelbes Heft erhalten haben. Die Lückenlosigkeit der Gelben Heft wird daher anhand von Kindern überprüft, deren letzte U-Untersuchung die U7a gewesen ist. 


```{r Lueckenlosigkeit_Heft_Def2, echo=FALSE, warning=FALSE, message=FALSE}
### data prep
...
lueckl_heft_def2 <- tmp
```

```{r Lueckenlosigkeit_Heft_Def2_output}
tmp <- lueckl_heft_def2 %>% 
  filter(u_nr == u_min) %>%
...
# Tabellenoutput putzen 
tmp2$Text <- c('Übermittelte Gelbe Hefte','Kopien liegen lückenlos vor', 
               'irgendwelche Eintragungen liegen lückenlos vor', 
               'Ergebnisseite liegt lückenlos vor', 'U lückenlos ausgefüllt (vereinfacht)', 
               'U lückenlos ausgefüllt (vollständig)')
...
```{r Lueckenlosigkeit_Heft_Def2_fehlendeUs, warning=FALSE, message=FALSE}
tmp <- lueckl_heft_def2
...
tbl2 <- tabyl(tmp$u[tmp$data==0], show_missing_levels = TRUE)
tbl3 <- tabyl(tmp$u[tmp$results==0], show_missing_levels = TRUE)
tbl4 <- tabyl(tmp$u[tmp$ges_easy==0], show_na = FALSE)
tbl5 <- tabyl(tmp$u[tmp$ges_hard==0], show_na = FALSE)
...
Einteilung in Gruppen: Kinder, bei denen U2-U7, U7a, U8 und U9 die letzte U-Untersuchung ist
```{r}
tmp <- lueckl_heft_def2
tmp$u <- as.factor(tmp$u)

tmp <- tmp %>% mutate(letzte_u = case_when(
  u_max %in% c(1,2,3,4,5,6) ~ 'U2-U7',
  u_max == 7 ~ 'U7a',
  u_max == 8 ~ 'U8', 
  u_max == 9 ~ 'U9'
))

tmp %>% tabyl(letzte_u, show_missing_levels = TRUE) %>% kbl2()
```

fehlende Kopien:
```{r}
tmp %>% filter(kopien==0) %>% tabyl(u, letzte_u, show_missing_levels = TRUE) %>% kbl2()
```

fehlende Daten:
```{r}
tmp %>% filter(data==0) %>% tabyl(u, letzte_u, show_missing_levels = TRUE) %>% kbl2()
```

fehlende Ergebnisseiten:
```{r}
tmp %>% filter(results==0) %>% tabyl(u, letzte_u, show_missing_levels = TRUE) %>% kbl2()
```
