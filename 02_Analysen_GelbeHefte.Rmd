...
mis_copy <- tmp %>% 
  #nur noch IDs mit Kopien behalten
  filter(kopien == 1) %>% 
  mutate(
    #bleiben nicht-aufeinenader folgende Us übrig?
    diff=u_nr-lag(u_nr),
    #Marker vergeben (später an Gesamtdatensatz dranhängen)
    lueckl=ifelse(any(diff>1),0,1),
    lueckl=ifelse(is.na(lueckl),1,0)) %>% 
  select(GH_ID, u, diff_copy=diff, lueckl_copy=lueckl)

# fehlende Angaben identifizieren
mis_data <- tmp %>% 
  filter(kopien == 1 & data == 1) %>% 
  mutate(diff=u_nr-lag(u_nr),
         lueckl=ifelse(any(diff>1),0,1), 
         lueckl=ifelse(is.na(lueckl),1,0)) %>% 
  select(GH_ID, u, diff_data=diff, lueckl_data=lueckl)

# fehlende Ergebnisseiten identifizieren
mis_res <- tmp %>% 
  filter(kopien == 1 & data == 1 & results == 1) %>% 
  mutate(diff=u_nr-lag(u_nr),
         lueckl=ifelse(any(diff>1),0,1), 
         lueckl=ifelse(is.na(lueckl),1,0)) %>% 
  select(GH_ID, u, diff_res=diff, lueckl_res=lueckl)
...
mis_ges_easy <- tmp %>% 
  ...
  mutate(diff=u_nr-lag(u_nr),
         lueckl=ifelse(any(diff>1),0,1), 
         lueckl=ifelse(is.na(lueckl),1,0)) %>% 
  select(GH_ID, u, diff_ges_easy=diff, lueckl_ges_easy=lueckl)
...
mis_ges_hard <- tmp %>% 
  ...
  mutate(diff=u_nr-lag(u_nr),
         lueckl=ifelse(any(diff>1),0,1), 
         lueckl=ifelse(is.na(lueckl),1,0)) %>% 
  select(GH_ID, u, diff_ges_hard=diff, lueckl_ges_hard=lueckl)

#Lückenlosigkeit Kopie & Daten dranhängen (lueckl==1 --> lückenlos)
tmp <- left_join(tmp, mis_copy, by=c('GH_ID', 'u')) %>%
  left_join(., mis_data, by=c('GH_ID', 'u')) %>%
  left_join(., mis_res, by=c('GH_ID', 'u')) %>%
  left_join(., mis_ges_easy, by=c('GH_ID', 'u')) %>%
  left_join(., mis_ges_hard, by=c('GH_ID', 'u')) %>%
  filter(kopien == 1 | data == 1) %>%
  select(GH_ID, u, contains('lueckl')) %>%
  ungroup()
